...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for images, labels in train_loader:
        ...
        # 计算损失（主损失 + 0.2*辅助损失，这里用0.2比原论文0.3更稳健）
        loss_main = criterion(outputs, labels)
        loss_aux1 = criterion(aux1, labels)
        loss_aux2 = criterion(aux2, labels)
        loss = loss_main + 0.2 * (loss_aux1 + loss_aux2)
        
        # 反向传播与优化
        loss.backward()
        optimizer.step()

        # 计算当前batch的训练准确率（依然只看主输出）
...
if __name__ == '__main__':
    ...
    # 初始化核心组件
    # --- aux_logits 改回 True ---
    model = GoogLeNet_CIFAR10(num_classes=10, aux_logits=True).to(device)
